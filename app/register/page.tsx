...
export default function RegisterPage() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 1. Registrera användaren i auth
      ...
      if (authData.user) {
        ...
        // 2. Skapa spelare i players-tabellen
        const { data: playerData, error: playerError } = await supabase
          .from('players')
          .insert([
            {
              user_id: authData.user.id, // Lägg till user_id för koppling
              name: formData.name,
              email: formData.email,
              phone: formData.phone
            }
        ...
        console.log("Player creation response:", playerData)
        if (playerError) {
